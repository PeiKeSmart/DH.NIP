...
| 格式化 | 忌全量重排、无差异格式化提交；不得仅为对齐批量移除/合并空白行 |
...
- 先列方案再实现
- 标记不确定上下文为"需查看文件"
- 修改或提交代码后，自动运行相关单元测试；未找到则说明，且不自动新增测试项目
